...
class LocalizationHelper {
    ...
    // Cache the sample scripts to avoid re-computing
    private var cachedSampleScripts: [(category: String, text: String)]?
    
    // Get localized sample scripts based on current locale
    func getLocalizedSampleScripts() -> [(category: String, text: String)] {
        // Return cached version if available
        if let cached = cachedSampleScripts {
            return cached
        }
        
        let locale = Locale.current.languageCode ?? "en"
        
        let scripts: [(category: String, text: String)]
        switch locale {
        case "zh":
            scripts = [
